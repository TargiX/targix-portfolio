...
I built this fast with an AI assistant, which is exactly where critical review earns its keep. The clearest example: the sleep slider rendered as an invisible track, with only the thumb showing, floating on white.
...
That's the recurring tax on AI-generated frontend: it's confident with modern APIs and quietly wrong at the edges. Reading the rendered DOM is what surfaced it.
